...
- SQL (`db/maes.sqlite3` via `db/schema.sql`) is the source of truth. Flag hand-edits to generated digests/curriculum/labs/transcripts as the primary fix path.
...
- Flag SSRF / unvalidated URL fetches in scouts, command injection in shell wrappers, and path traversal when reading markdown under `digests/`, `curriculum/`, or `labs/`.

...
2. **Read each digest file** at the `markdown_path` returned. Optionally skim any `digests/forecast-*.md` from the last 7 days if present; do not invent forecast claims.
...
4. **Save** to `wiki/week-YYYY-MM-DD.md` (create `wiki/` if absent; gitignored). Use today's date (or the period end) in the filename.
5. **Record the run:**
   ```
   python3 scripts/db.py insert-wiki \
     ...
     --markdown-path wiki/week-YYYY-MM-DD.md \
     --digest-count <N>
   ```
